    ...
    try:
        ...
        if photos.total_count > 0:
            ...
            if r.status_code == 200:
                ...
                av = av.resize((120, 120), Image.Resampling.LANCZOS)
        ...
        pass
    fb = Image.new("RGBA", (120, 120), (20, 30, 50, 255))
    d = ImageDraw.Draw(fb)
    d.ellipse((0, 0, 120, 120), outline=(0, 255, 200), width=3)
    f = get_safe_font(50)
...
async def tempest_ai(m: Message):
    ...
    with sqlite3.connect("data/bot.db") as conn:
        c = conn.cursor()
        c.execute("SELECT role, content FROM conversation_history WHERE user_id = ? ORDER BY rowid DESC LIMIT 20", (uid,))
        hist = c.fetchall()
        for role, content in reversed(hist):
            conversation_memory[uid].append({"role": role, "content": content})
    ...
    if GROQ_API_KEY:
        try:
            ...
            with sqlite3.connect("data/bot.db") as conn:
                conn.execute("INSERT INTO conversation_history (user_id, role, content) VALUES (?, 'user', ?)", (uid, query))
                conn.execute("INSERT INTO conversation_history (user_id, role, content) VALUES (?, 'assistant', ?)", (uid, resp))
                conn.commit()
