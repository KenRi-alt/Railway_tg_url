#!/usr/bin/env python3
# ========== TEMPEST BOT - ULTIMATE FIXED EDITION ==========
# Version: 4.0 - All Commands Fixed & Optimized
...
async def dice_cmd(message: Message):
    ...
    msg = await message.answer("🎲 <b>Rolling...</b>", parse_mode=ParseMode.HTML)
    faces = ["⚀", "⚁", "⚂", "⚃", "⚄", "⚅"]
    roll = random.randint(1, 6)
    
    for i in range(3):
        await msg.edit_text(f"🎲 <b>Rolling...</b> {random.choice(faces)}", parse_mode=ParseMode.HTML)
        await asyncio.sleep(0.3)
    
    await msg.edit_text(f"🎲 <b>You rolled:</b> {faces[roll-1]} ({roll})", parse_mode=ParseMode.HTML)
...
async def flip_cmd(message: Message):
    ...
    for i in range(3):
        await msg.edit_text(f"🪙 <b>Flipping...</b>", parse_mode=ParseMode.HTML)
        await asyncio.sleep(0.3)
...
async def tempest_story_cmd(message: Message):
    ...
    for i, chapter in enumerate(chapters):
        ...
        await asyncio.sleep(6)  # 6 seconds per chapter
...
async def fate_cmd(message: Message):
    ...
    # Check if there's an existing fate pair for today
    conn = sqlite3.connect("data/bot.db")
    c = conn.cursor()
    c.execute("""
    ...
    if existing:
        await message.answer(
            ...
            f"💖 {existing[2]}% Love\n\n"
            f"<i>Fate has already decided! Try again in 24 hours.</i>",
            parse_mode=ParseMode.HTML
        )
        conn.close()
        return
    ...
    if len(members) < 2:
        await msg.edit_text("❌ Not enough members! Need at least 2 active users.")
        return
...
async def broadcast_cmd(message: Message):
    ...
    await message.answer("📢 <b>Send me text, photo, or video to broadcast!</b>\n❌ /cancel to stop", parse_mode=ParseMode.HTML)
