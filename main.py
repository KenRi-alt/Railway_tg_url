...
def init_db():
    with sqlite3.connect("data/bot.db") as conn:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS command_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp TEXT, user_id INTEGER, chat_id INTEGER, command TEXT, success INTEGER, error TEXT)''')
        c.execute('''CREATE TABLE IF NOT EXISTS error_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp TEXT, command TEXT, error TEXT)''')
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS saved_handlers (id INTEGER PRIMARY KEY AUTOINCREMENT, handler_code TEXT, saved_date TEXT)''')
...
async def handle_common(message: Message, cmd: str):
    ...
    if chat.type in [ChatType.GROUP, ChatType.SUPERGROUP]:
        try:
            with sqlite3.connect("data/bot.db") as conn:
                c = conn.cursor()
                c.execute("INSERT OR IGNORE INTO groups (group_id, title, joined_date) VALUES (?, ?, ?)",
                         (chat.id, str(chat.title), datetime.now().isoformat()))
                c.execute("UPDATE groups SET title = ? WHERE group_id = ?",
                         (str(chat.title), chat.id))
                conn.commit()
        except:
            pass
...
async def fetch_user_avatar(user_id: int, first_name: str = "User") -> Image.Image:
    ...
    initial = first_name[0].upper() if first_name else "T"
...
async def start_cmd(message: Message):
    ...
    if not user:
        return
    await message.answer(f"{header('TEMPEST GUIDER')}\nWelcome {user.first_name}!\n\n/help - Commands")
...
async def wish_cmd(message: Message):
    ...
    if len(args) < 2:
        ...
        return
    msg = await message.answer("Consulting the storm...")
...
async def fate_cmd(message: Message):
    ...
    with sqlite3.connect("data/bot.db") as conn:
        ...
        existing = c.fetchone()
        if existing:
            await msg.edit_text(f"Today: {existing[0]} & {existing[1]} ({existing[2]}%)")
            return
...
async def profile_cmd(message: Message):
    ...
    await message.answer_photo(
        ...
        caption=f"{user.first_name}'s Tempest ID Card"
    )
...
async def encrypt_cmd(message: Message):
    ...
    if len(args) < 2:
        ...
        return
    await message.answer(f"<code>{EncryptionEngine.encrypt(args[1])}</code>", parse_mode=ParseMode.HTML)
...
async def decrypt_cmd(message: Message):
    ...
    await message.answer(dec if dec else "Invalid!")
...
async def tempest_join_cmd(message: Message):
    ...
    msg = await message.answer("Preparing ritual...")
    for text in ritual:
...
async def tempest_story_cmd(message: Message):
    ...
    chapters = [
        ...
        ("Chapter 2: Guild of Shadows", "With Bablu holding the vanguard and Ravijah orchestrating tactics, Keny forged the Tempest Guild."),
    ...
    msg = await message.answer("Opening Tempest Archives...")
    await asyncio.sleep(1.5)
    for i, (title, content) in enumerate(chapters):
        progress = "[" + "#" * (i + 1) + "." * (len(chapters) - i - 1) + "]"
        await msg.edit_text(f"Loading... {progress} {i+1}/{len(chapters)}")
...
async def curse_cmd(message: Message):
    ...
    with sqlite3.connect("data/bot.db") as conn:
        ...
        conn.commit()
    await message.reply(f"{target.first_name} is cursed!")
...
async def time_cmd(message: Message):
    ...
    if len(args) < 2 or args[1].lower().strip() not in COUNTRY_TIMEZONES:
        await message.answer("Usage: /time [country]")
        return
    ...
    try:
        ...
        await message.answer(f"{country.upper()}: {now.strftime('%H:%M:%S')}")
    except:
...
async def ping_cmd(message: Message):
    ...
    if user.id != OWNER_ID and not await is_admin(user.id):
        await message.answer("Admin only!")
        return
    await message.answer(f"Pong! Uptime: {format_uptime(int(time.time() - start_time))}")
...
async def stats_cmd(message: Message):
    ...
    with sqlite3.connect("data/bot.db") as conn:
        ...
        alive = c.fetchone()[0]
        c.execute("SELECT COUNT(*) FROM users WHERE last_active < ? OR last_active IS NULL", ((datetime.now() - timedelta(days=7)).isoformat(),))
        dead = c.fetchone()[0]
    await message.answer(f"Total: {total}\nAlive (7d): {alive}\nDead: {dead}")

@dp.message(Command("scan"))
async def scan_cmd(message: Message):
    await handle_common(message, "scan")
    await message.answer("Scan complete!")
...
async def users_cmd(message: Message):
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        f.write("=" * 40 + "\n\n")
        for uid, name, uname, up, cmds, banned in users:
...
async def broadcast_cmd(message: Message):
    ...
    await message.answer("Send me anything to broadcast!")
...
async def lag_cmd(message: Message):
    await handle_common(message, "lag")
    await message.answer("Lag test complete!")
...
async def query_cmd(message: Message):
    ...
    code = args[1]
    with sqlite3.connect("data/bot.db") as conn:
        conn.execute("INSERT INTO saved_handlers (handler_code, saved_date) VALUES (?, ?)",
...
async def maintenance_cmd(message: Message):
    ...
    await message.answer(f"Maintenance: {'ON' if maintenance_mode else 'OFF'}")
...
async def logs_cmd(message: Message):
    ...
    with sqlite3.connect("data/bot.db") as conn:
        c = conn.cursor()
        c.execute("SELECT timestamp, user_id, chat_id, command, success FROM command_logs ORDER BY id DESC LIMIT 100")
        cmd_logs = c.fetchall()
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        f.write("-" * 30 + "\n")
        for ts, uid, cid, cmd, success in cmd_logs:
            f.write(f"Time: {ts}\nUser: {uid}\nChat: {cid}\nCommand: {cmd}\nSuccess: {success}\n")
            f.write("-" * 20 + "\n")
        f.write("\nERROR LOGS:\n")
        f.write("-" * 30 + "\n")
...
async def ban_cmd(message: Message):
    user, chat = await handle_common(message, "ban")
    if not user:
        return
    if user.id != OWNER_ID and not await is_admin(user.id):
        await message.answer("Admin only!")
        return
    if not message.reply_to_message:
        await message.answer("Reply to ban!")
        return
    target = message.reply_to_message.from_user
    ...
    with sqlite3.connect("data/bot.db") as conn:
        ...
        conn.commit()
    await message.reply(f"{target.first_name} banned!")

@dp.message(Command("unban"))
async def unban_cmd(message: Message):
    user, chat = await handle_common(message, "unban")
    if not user:
        return
    if user.id != OWNER_ID and not await is_admin(user.id):
        await message.answer("Admin only!")
...
async def banlist_cmd(message: Message):
    user, chat = await handle_common(message, "banlist")
    if not user:
        return
    if user.id != OWNER_ID and not await is_admin(user.id):
        await message.answer("Admin only!")
    ...
    if GROQ_API_KEY:
        try:
            ...
            system_prompt = (
                ...
                "Your creator is @Nocis_creed (Keny) - https://t.me/Nocis_creed. "
                "Your assistant developer is @dont_try_to_copy_mee (Ravijah) - https://t.me/dont_try_to_copy_mee. "
