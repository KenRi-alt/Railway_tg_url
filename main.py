...
print("=" * 60)
print("TEMPEST GUIDER - COMPLETE FINAL")
print("=" * 60)
...
async def upload_to_catbox(data, filename):
    ...
    try:
        ...
        print(f"Catbox status: {r.status_code}")
        print(f"Catbox response: {r.text[:200]}")
...
def gen_profile_card(username, uid, rank, up, ws, av):
    ...
    draw_visible_text(d, (230, 70), str(username)[:15], fn, (255, 255, 255))
    draw_visible_text(d, (230, 150), f"ID: {uid}", fi, (180, 200, 230))
    draw_visible_text(d, (230, 210), f"Rank: {rank}", fi, (0, 255, 200))
    draw_visible_text(d, (230, 280), f"Uploads: {up}  |  Wishes: {ws}", fi, (255, 215, 0))
    draw_visible_text(d, (970, 460), "TEMPEST GUIDER", fi, (100, 116, 139), anchor="ra")
    b = io.BytesIO()
...
def gen_fate_card(n1, n2, pct, q, av1=None, av2=None):
    ...
    fq = get_safe_font(22)
    if av1:
        img.paste(av1, (120, 70), mask=av1)
    if av2:
        img.paste(av2, (730, 70), mask=av2)
    draw_visible_text(d, (500, 40), "TEMPEST FATE MATRIX", ft, (255, 100, 180), anchor="mm")
    draw_visible_text(d, (500, 260), f"{n1}  +  {n2}", fn, (255, 255, 255), anchor="mm")
    draw_visible_text(d, (500, 320), f"{pct}% COMPATIBILITY", fn, (255, 215, 0), anchor="mm")
    words = q.split()
    lines = []
    cur = ""
    for wrd in words:
    ...
    yp = 390
    for ln in lines[:5]:
        draw_visible_text(d, (500, yp), ln, fq, (210, 220, 240), anchor="mm")
        yp += 35
    b = io.BytesIO()
    img.save(b, format="PNG")
    b.seek(0)
...
async def tempest_story_cmd(m: Message):
    ...
    if not u:
        return
    chapters = [
        ("📖 Chapter 1", "Keny Marcus opened his eyes in the obsidian realm alongside Bablu and Ravijah."),
        ("⚔️ Chapter 2", "With Bablu holding vanguard and Ravijah orchestrating, Keny forged the Guild."),
        ("⚡ Chapter 3", "Ravijah bypassed the firewall while Bablu smashed the gates."),
        ("👑 Chapter 4", "Standing atop the spire, the three gazed across the grid."),
        ("🌌 Chapter 5", "The Void whispered promises of infinite power."),
        ("🔱 Chapter 6", "Lightning became blood. Keny, Ravijah, Bablu - eternal storm."),
    ]
...
async def ping_cmd(m: Message):
    ...
    if u.id != OWNER_ID and not await is_admin(u.id):
        await m.answer("🚫 Admin only!")
        return
    await m.answer(f"🏓 Uptime: {format_uptime(int(time.time() - start_time))}")
